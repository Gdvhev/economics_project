# -*- coding: utf-8 -*-
"""
Created on Sat Apr 25 10:44:23 2020
...
def chance_util_second(i_map, history, card_1, card_2):
    ...
    #Case in which the players have different cards. So the possibilities are 8 (5 different types of cards left) 
    else:
        expected_value = 0
        n_possibilities = 8
        for i in range(_N_CARDS):
                if i!= card_1 and i != card_2:
                   expected_value += 2 * cfr(i_map, history + "n", card_1, card_2, 1, 1, 1/n_possibilities, i)
                else:
...
class InformationSet():
    def __init__(self, key):
        self.key = key
        if(key.endswith("r") and not key.endswith("rr")):
            self.regret_sum = np.zeros(_N_ACTIONS_RAISE)
            self.strategy_sum = np.zeros(_N_ACTIONS_RAISE)
            self.strategy = np.repeat(1/_N_ACTIONS_RAISE, _N_ACTIONS_RAISE)
        else:
            self.regret_sum = np.zeros(_N_ACTIONS_STANDARD)
            self.strategy_sum = np.zeros(_N_ACTIONS_STANDARD)
            self.strategy = np.repeat(1/_N_ACTIONS_STANDARD, _N_ACTIONS_STANDARD)
    ...
    def calc_strategy(self):
        ...
        if total > 0:
            ...
            if(self.key.endswith("r") and not self.key.endswith("rr")):
                n = _N_ACTIONS_RAISE
                strategy = np.repeat(1/n, n)
            else:
                n = _N_ACTIONS_STANDARD
                strategy = np.repeat(1/n, n)
