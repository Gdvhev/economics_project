...
def gen_strats(filename,n_iterations):
    ...
    output_strategy("strategy.txt",infosets,infoset_id_of,id_dic,val,len(infosets),len(fake_infosets))
...
def output_strategy(filename,infosets,infoset_id_of,id_dic,val,infon,fakeinfon):
    f = open(filename,"w")
    ...
    p1_count=0
    for nodes in infosets.values():
        node=id_dic[nodes[0]]
        if(not node.isNature() and node.player_id=="1"):
            p1_count+=1

    f.write("#1 has %d infosets\n"%(p1_count))
    #print(infosets)
